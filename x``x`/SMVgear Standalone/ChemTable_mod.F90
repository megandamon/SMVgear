...
!#include "smv2chem_par.h"	
	...
	Subroutine CalcTabl(Tabl,gCT,ncs,ncsp)
      use Smv2Chem2_mod
#include "smv2chem_par.h"
      ...
      nfdh3   = numRxnsThreeActiveReactant(gCT)
      nfdl2   = nfdh3  + 1
      nfdh2   = nfdh3  + numRxnsTwoActiveReactant(gCT)
      nfdl1   = nfdh2  + 1
      nfdh1   = nfdh2  + numRxnsOneActiveReactant(gCT)
      nfdl0   = nfdh1  + 1
      nallr   = nallrat(gCT)
      ...
      do nkn = 1, nallr
        nk = oldRxnRateNum(nkn,gCT)
        Tabl%RxnIn(1,nkn) = newSpcNumActiveProduct(1,nk,gCT)
        Tabl%RxnIn(2,nkn) = newSpcNumActiveProduct(2,nk,gCT)
        Tabl%RxnIn(3,nkn) = newSpcNumActiveProduct(3,nk,gCT)
      end do

		!Calculate reaction table from spc->reaction mapping
		do i=npllo(ncsp),nplhi(ncsp)
			jspc = jspnpl(i)
			do k=npl5(i),nph1(i)
				SpcRxnNum(jspc) = SpcRxnNum(jspc)+1
				SpcRxn(jspc,SpcRxnNum(jspc)) = reOrderedRxnRateEachLossA(k)
			end do
			do k=npl5(i),nph2(i)
				SpcRxnNum(jspc) = SpcRxnNum(jspc)+1
				SpcRxn(jspc,SpcRxnNum(jspc)) = reOrderedRxnRateEachLossB(k)
			end do
			do k=npl5(i),nph3(i)
				SpcRxnNum(jspc) = SpcRxnNum(jspc)+1
				SpcRxn(jspc,SpcRxnNum(jspc)) = reOrderedRxnRateEachLossC(k)
			end do
			do k=npl5(i),nph4(i)
				SpcRxnNum(jspc) = SpcRxnNum(jspc)+1
				SpcRxn(jspc,SpcRxnNum(jspc)) = reOrderedRxnRateEachLossD(k)
			end do
			do k=npl5(i),nph5(i)
				SpcRxnNum(jspc) = SpcRxnNum(jspc)+1
				SpcRxn(jspc,SpcRxnNum(jspc)) = reOrderedRxnRateEachLossE(k)
			end do
		end do
...
!   Subroutine PrintChem(Tabl,Reorder)
!      use Smv2Chem1_mod
...
!!origSpcNumber(gasChemType=1,jreord) = inewold from common
!
!      nfrac = 0
