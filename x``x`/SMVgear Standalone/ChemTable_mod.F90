...
#include "smv2chem2.h"
      ...
      nfdh3   = ithrr(gCT)
      nfdl2   = nfdh3  + 1
      nfdh2   = nfdh3  + itwor(gCT)
      nfdl1   = nfdh2  + 1
      nfdh1   = nfdh2  + ioner(gCT)
      nfdl0   = nfdh1  + 1
      nallr   = nallrat(gCT)
      ...
      do nkn = 1, nallr
        nk = noldfnew(nkn,gCT)
        Tabl%RxnIn(1,nkn) = irm2(1,nk,gCT)
        Tabl%RxnIn(2,nkn) = irm2(2,nk,gCT)
        Tabl%RxnIn(3,nkn) = irm2(3,nk,gCT)
      end do
		...
		do i=npllo(ncsp),nplhi(ncsp)
			...
			do k=npl5(i),nph1(i)
				...
				SpcRxn(jspc,SpcRxnNum(jspc)) = lossra(k)
			end do
			do k=npl5(i),nph2(i)
				SpcRxnNum(jspc) = SpcRxnNum(jspc)+1
				SpcRxn(jspc,SpcRxnNum(jspc)) = lossrb(k)
			end do
			do k=npl5(i),nph3(i)
				SpcRxnNum(jspc) = SpcRxnNum(jspc)+1
				SpcRxn(jspc,SpcRxnNum(jspc)) = lossrc(k)
			end do
			do k=npl5(i),nph4(i)
				SpcRxnNum(jspc) = SpcRxnNum(jspc)+1
				SpcRxn(jspc,SpcRxnNum(jspc)) = lossrd(k)
			end do
			do k=npl5(i),nph5(i)
				SpcRxnNum(jspc) = SpcRxnNum(jspc)+1
				SpcRxn(jspc,SpcRxnNum(jspc)) = lossre(k)
			end do
		end do
