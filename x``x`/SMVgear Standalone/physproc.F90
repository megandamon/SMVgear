...
!-----------------------------------------------------------------------------
      ...
      subroutine physProc  &
     &  (doQqjkInChem, doSurfEmissInChem, prQqjk, prSmv2, numLats,  &
...
!         ================
          call Solve_Block  &
!         ================
     &      (doQqjkInChem, doSurfEmissInChem, prQqjk, prSmv2, ifSun,  &
...
!   numZones    : # of zones (numLongs * numLats * numVert)
!   numGridCellsInBlock    : intended # of grid-cells in a grid-block
!   gasChemistryType       : identifies gas chemistry type (1..gasChemistryTypeGAS)
!   doCellChem : do chemistry for a particular cell?
!   ntLoopNcs : tbd
!   prate     : photolysis rate constants (s^-1)
...
!-----------------------------------------------------------------------------
     ...
     &  (iday, idaynt, numZones, numGridCellsInBlock, gasChemistryType, doCellChem, ntLoopNcs,  &
...
!     ----------------------
      ...
      integer, intent(out) :: jreorder(numZones)
...
!     ----------------
     ...
     &          doCellChem(jloop)) then
      ...
      if (nblockuse > nblockuse_max) then
        nblockuse_max = nblockuse
!c      Write (6,*) 'nblockuse_max:  ', nblockuse_max
      end if

      if (nblockuse_max > MXBLOCK) then
...
!     corig  : original gas-phase cogasChemistryType used to restart Smvgear if a failure
!              occurs (molec/cm^3)
...
!         ============
          call Smvgear  &
!         ============
     &      (doQqjkInChem, doSurfEmissInChem, prQqjk, prSmv2,  &
...
!     ----------------------
      ...
      integer :: kloop
      integer :: gasChemistryTypep            ! gasChemistryType       => for daytime   gas chemistry
...
!     ---------------------------------------------------------------
      ...
      do i = 1, nm3bod(gasChemistryType)
        nk   = nreac3b (i,gasChemistryType)
        jold = lgas3bod(i,gasChemistryType)
        do kloop = 1, ktloop
...
!     -----------------------------------------------------------
      ...
      do i = 1, nmoth(gasChemistryType)
        nk   = nreacoth(i,gasChemistryType)
        jold = lgasbino(i,gasChemistryType)
        do kloop = 1, ktloop
          rrate(kloop,nk) = rrate(kloop,nk) * cblk(kloop,jold)
       end do
      end do
...
!     ---------------------------------------------------------------
!     Reset gasChemistryTypep; multiply photorate coef. (s^-1) by concentration
!     (molec/cm^3) when photodissociating species is inactive.  Thus,
!     rate does not need to be multiplied later by concentration.
!     ---------------------------------------------------------------

      gasChemistryTypep = (ifSun - 1) * ICS + gasChemistryType

      do i = 1, nolosp(gasChemistryTypep)
        nk   = nknlosp(i,gasChemistryType)
        j    = jphotnk(nk,gasChemistryType)
        jold = losinacp(i,gasChemistryType)
        do kloop = 1, ktloop
